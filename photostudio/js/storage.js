...
const SEED_STUDIOS = [
  { id: 's1', ownerId: 'u2', name: 'Golden Light Studio',    zone: 'maadi',     lat: 29.9602, lng: 31.2569, description: 'A bright, airy space with floor-to-ceiling windows. Perfect for portrait and fashion shoots.', price: 120, capacity: 8,  available: true, features: ['Natural Light', 'Backdrop Wall', 'Dressing Room'], createdAt: '2024-01-05T00:00:00.000Z' },
  { id: 's2', ownerId: 'u2', name: 'Dark Elegance Studio',   zone: 'zamalek',   lat: 30.0626, lng: 31.2197, description: 'A moody studio with professional LED lighting rigs. Great for dramatic portraits.',             price: 150, capacity: 6,  available: true, features: ['LED Rigs', 'Fog Machine', 'Music System'],      createdAt: '2024-01-06T00:00:00.000Z' },
  { id: 's3', ownerId: 'u2', name: 'Rooftop Skyline Studio', zone: 'nasr-city', lat: 30.0626, lng: 31.3361, description: 'An outdoor rooftop studio with a stunning city skyline backdrop.',                               price: 200, capacity: 10, available: true, features: ['Outdoor', 'Skyline View', 'Sunset Hours'],     createdAt: '2024-01-07T00:00:00.000Z' },
  { id: 's4', ownerId: 'u2', name: 'Cairo Frame Studio',     zone: 'new-cairo', lat: 30.0254, lng: 31.4915, description: 'A modern minimalist studio in New Cairo with state-of-the-art equipment.',                        price: 180, capacity: 8,  available: true, features: ['Cyclorama Wall', 'Green Screen', 'Edit Suite'],  createdAt: '2024-01-08T00:00:00.000Z' },
];

const SEED_BOOKINGS = [];

const STORAGE_VERSION = '3'; // bump to force re-seed when schema changes

function initStorage() {
  if (localStorage.getItem('photostudio_version') !== STORAGE_VERSION) {
    ...
    localStorage.setItem('photostudio_version', STORAGE_VERSION);
  }
}
